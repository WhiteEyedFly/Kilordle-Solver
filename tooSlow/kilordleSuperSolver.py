...
Solving based on the words is too slow, so solve based on the letterPositions instead
Loop over the letter positions, selecting a random word that covers it
Do so 10000 times, choose the smallest solution
"""
...
def reducer(acceptableWords, answers, wordList, letterPositions, solution, solutions, setsChecked):
    ...
    # We know already that a 34 solution is possible
    if len(solution) > 34:
        return solutions
    ...
    #if len(solutions) // 100 == len(solutions) / 100:
    #    print(str(len(solutions)) + " solutions have been found")
    #    print(setsChecked[4])
    return solutions
...
def main():
    ...
    solutions = reducer(starterWords, WORDS, WORDLES, letterPositions, [], [], sets)
